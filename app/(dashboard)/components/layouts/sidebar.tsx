        ...
        <aside className="w-80 min-h-screen bg-white border-r border-gray-100 flex flex-col fixed left-0 top-0">
            ...
            <div className="flex flex-col gap-2 mt-12 p-5">
                {menuItems.map((item, index) => {
                    ...
                    return (
                        <Link href={item.link} key={index} className={`flex gap-3 item-center py-3 px-4.5 rounded-lg font-medium duration-300 ${
                            isActive ? "bg-primary/17 text-primary" : "hover:bg-gray-100"
                            }`}
                        >
                            <item.icon size={24} />
                            <span>{item.name}</span>
            ...
            <button className="flex gap-3 cursor-pointer font-medium py-3 px-4.5 mx-5 hover:bg-gray-100 duration-300 rounded-lg mt-auto mb-10" onClick={handleLogout}>
                <FiLogOut size={24} />
                Log Out
            </button>
        </aside>
